...
function HeroContent() {
  return (
    <div className="relative z-10 flex flex-col items-center text-center">
      ...
      <p
        className="hero-rise group/name inline-block text-sm font-medium tracking-tight text-ink-soft sm:text-lg transition-all duration-300 cursor-pointer hover:scale-110 hover:-translate-y-1"
        style={{ animationDelay: `${STEP.intro}ms` }}
      >
        Hi, I&apos;m{" "}
        <span className="font-bold text-white transition-all duration-300 group-hover/name:text-transparent group-hover/name:bg-clip-text group-hover/name:bg-linear-to-r group-hover/name:from-white group-hover/name:via-accent-soft group-hover/name:to-accent group-hover/name:drop-shadow-[0_0_25px_rgba(139,147,255,0.9)]">
          Waqas Zafar
        </span>
      </p>
      <p
        className="hero-rise mt-2 text-[0.8125rem] font-medium uppercase tracking-[0.18em] text-ink-muted"
      ...
      {/* Headline */}
      <h1
        className="hero-rise mt-6 max-w-[15ch] text-balance text-4xl font-semibold leading-[1.06] tracking-tight text-ink sm:max-w-[18ch] sm:text-5xl md:text-6xl lg:text-[4.25rem]"
        style={{ animationDelay: `${STEP.headline}ms` }}
      >
        AI Powered &amp; Saas{" "}
        <span className="text-accent-soft">Solution for Startups</span>
      </h1>

      {/* Supporting line */}
      <p
        className="hero-rise mt-6 max-w-xl text-pretty text-base leading-relaxed text-ink-soft sm:text-lg"
      ...
      >
        ...
        <a
          href="#contact"
          className={`inline-flex w-full items-center justify-center rounded-full border border-line-strong bg-white/[0.03] px-7 py-3.5 text-sm font-semibold tracking-tight text-ink backdrop-blur-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-white hover:bg-white hover:text-[#08090c] hover:shadow-[0_0_30px_rgba(255,255,255,0.35)] sm:w-auto ${FOCUS_RING}`}
        >
          Let&apos;s Talk
        </a>
      </div>
